<template>
    <div class="page-container">
      <template v-if="!loading">
        ...
        >
          <template v-if="!store.getTradesByStrategy(strat._id)">
            <loading-page :strat="strat" />
          </template>
          <template v-else>
            <v-row>
            ...
            <v-row>
              <v-col class="table-header">
                <v-btn
                  size="large"
                  variant="outlined"
                  :id="'menu-activator-' + strat._id"
                >
                  Add Trades
                </v-btn>
                <v-menu :activator="'#menu-activator-' + strat._id">
                  <v-list>
                    <v-list-item
                      @click="handleAddTrade(strat)"
                      :key="1"
                      :value="1"
                    >
                    ...
                    <v-list-item
                      @click="handleImportTrades(strat)"
                      :key="2"
                      :value="2"
                    >
            ...
            <div
              ...
              :ref="(el) => (refsObject[strat._id] = el)"
            >
              <table v-if="!loading">
                <thead>
                  <open-trades-table-header :strat="strat" />
                </thead>
                <tbody>
                  <open-trades-row
                    v-for="trade in store.getTradesByStrategy(strat._id)"
                    :trade="trade"
                    :key="trade._id"
                    :strat="strat"
              ...
              <sticky-wrapper :parent="refsObject[strat._id]">
                <multi-select-menu-wrapper :storeId="strat._id">
                  <multi-select-menu
                    :strat="strat"
